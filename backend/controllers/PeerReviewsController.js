...
export const submitReview = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "All scores and comments are required" });
    }
    ...
    for (const score of scores) {
      if (score < 1 || score > 10) {
        return res
          .status(400)
          .json({ error: "All scores must be between 1 and 10" });
      }
    }
    ...
    if (comments.trim().length < 10) {
      return res
        .status(400)
        .json({ error: "Comments must be at least 10 characters" });
    }
    ...
    if (submission.author_id === reviewerId) {
      return res
        .status(403)
        .json({ error: "You cannot review your own submission" });
    }
    ...
    const existingReview = await pool
      ...
      .query(
        "SELECT review_id FROM Peer_Reviews WHERE submission_id = @submissionId AND reviewer_id = @reviewerId",
      );
    ...
    const result = await pool
      ...
      .input("status", sql.NVarChar(20), "Submitted").query(`
    ...
    if (
      error.code === "EREQUEST" &&
      error.message.includes("uq_reviews_user_submission")
    ) {
      return res
        .status(409)
        .json({ error: "You have already reviewed this submission" });
...
export const getUserReview = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .input("submissionId", sql.Int, Number(submissionId))
      .input("reviewerId", sql.Int, Number(reviewerId)).query(`
        SELECT 
          pr.review_id,
          pr.technical_score,
          pr.methodology_score,
...
export const getAggregateScores = async (req, res) => {
  try {
    ...
    console.error(
      "[PeerReview] Error fetching aggregate scores:",
      error.message,
    );
