...
const ShareMenu = ({ isOpen, onClose, postId, onShare }) => {
  ...
  const shareOptions = [
    ...
    },
    {
      ...
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.054 2.25H8.92l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        </svg>
      ),
    },
    {
...
const FeedCard = ({ post, onInteract }) => {
  ...
  return (
    ...
    >
      {!isInteractive && (
        <div
          className="absolute top-3 right-3 font-code text-[9px] uppercase tracking-widest px-2 py-0.5 rounded"
          style={{
            background:
              post.submissionStatus === "Pending"
                ? "rgba(245,158,11,0.15)"
                : post.submissionStatus === "Archived"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <span
              ...
              style={{ color: "#4B5563" }}
            >
              Posted by <span style={{ color: "#374151" }}>{post.user}</span>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span
              ...
              style={{
                background:
                  post.submissionStatus === "Published"
                    ? "rgba(34,197,94,0.08)"
                    : post.submissionStatus === "Pending"
      ...
      >
        <button
          onClick={() =>
            isInteractive ? navigate(`/post/${post.id}`) : undefined
          }
          disabled={!isInteractive}
