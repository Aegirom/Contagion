...
const toFeedPost = (submission, likedPosts, savedPosts) => ({
  ...
  family:
    submission?.malware_family ||
    submission?.malware_category ||
    submission?.template_type ||
    "Analysis",
  threat: severityFromCategory(
    submission?.malware_category || submission?.malware_family,
  ),
  ...
  isLiked:
    likedPosts?.[submission?.submission_id || submission?.id] !== undefined,
  isSaved:
    savedPosts?.[submission?.submission_id || submission?.id] !== undefined,
...
const FeedPage = () => {
  ...
  const posts = useMemo(() => {
    ...
    if (sortBy === "hot")
      return [...rows].sort(
        (a, b) => b.comments + b.score - (a.comments + a.score),
      );
    return rows;
        ...
        <div className="space-y-2">
          ...
          {!loading &&
            posts.map((post) => (
              <FeedCard
                key={post.id}
                post={post}
                onInteract={() => setSubmissions((prev) => [...prev])}
              />
            ))}
          ...
          {!loading && posts.length === 0 && (
            <div className="py-24 text-center rounded-xl border border-dashed border-phantom bg-obsidian/30">
              ...
              <p className="font-code text-xs uppercase tracking-widest text-gray-600 mb-2">
                No submissions in the feed yet
              </p>
              <p className="font-code text-[10px] text-gray-500">
                Published analysis reports will appear here
              </p>
            </div>
          )}
        </div>
