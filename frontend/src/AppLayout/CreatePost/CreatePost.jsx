...
import { createSubmission, evaluateSandboxFile, getSubmissionById, updateSubmission } from "../../services/api";
...
const CreatePost = () => {
  ...
  const { selectedFile, uploadedArtifact, uploadSelectedArtifact, clearUpload } = useArtifactStore();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionId = response.data.submission_id || response.data.submission?.submission_id || draftId;

      if (runSandbox && submitStatus !== "Draft" && artifact?.sha256_hash && submissionId) {
        try {
      ...
      setSubmitError(error.message || error.response?.data?.error || "Failed to submit analysis.");
    } finally {
      ...
      <div className="max-w-2xl mx-auto py-12 px-4 sm:px-6">
        <div className="mb-8">
          <h2
            ...
            style={{ color: "#F1F5F9" }}
          >
            {draftId ? "Edit" : "New"} <span style={{ color: "#22C55E" }}>Analysis Report</span>
          </h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            <label className="flex items-center justify-between rounded-lg border border-white/5 bg-[#0A0B10] px-4 py-3">
              <span>
                <span className="block font-code text-[10px] uppercase tracking-widest" style={{ color: "#F1F5F9" }}>
            ...
            <div className="flex gap-4">
              <button
                ...
                name="status"
                value="Draft"
                className="flex-1 py-4 rounded-lg font-display text-sm font-bold tracking-[0.2em] uppercase transition-all border"
                style={{
              ...
              <button
                ...
                className="flex-1 py-4 rounded-lg font-display text-sm font-bold tracking-[0.2em] uppercase transition-all disabled:opacity-50"
