...
const CountTile = ({ label, value, accent = "text-gray-900" }) => (
    ...
    <div className="rounded border border-phantom bg-obsidian/60 p-3">
      ...
      <div className="max-h-32 overflow-y-auto space-y-1">
        {items.slice(0, maxShow).map((item, i) => {
          ...
          return (
            <div key={i} className="flex items-center justify-between gap-2 py-1 px-2 rounded hover:bg-gray-100">
              ...
              {meta && (
                <span className="flex-shrink-0 font-code text-[9px] text-gray-500 truncate max-w-[120px]">
                  {meta}
                </span>
              )}
            </div>
        ...
        {items.length > maxShow && (
          <p className="font-code text-[10px] text-gray-500 pt-1 text-center">
...
const DetectionList = ({ stats }) => {
  ...
  return (
    <div className="rounded border border-phantom bg-obsidian/60 p-3">
      <p className="font-code text-[10px] uppercase tracking-[0.18em] text-gray-600 mb-2">
        Engine Verdicts
      </p>
...
function SandboxPage() {
  ...
  return (
    <main className="flex-1 min-w-0 overflow-x-hidden overflow-y-auto relative z-10 px-6 py-10 text-gray-900 md:px-8 lg:px-12">
