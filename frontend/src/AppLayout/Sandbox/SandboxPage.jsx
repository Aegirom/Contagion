...
import { evaluateSandboxFile, getSandboxExecutions, getSandboxSubmissions } from '../../services/api';
...
const CountTile = ({ label, value, accent = 'text-slate-100' }) => (
  <div className="rounded border border-phantom bg-obsidian/80 px-4 py-3">
    <p className="font-code text-[10px] uppercase tracking-[0.2em] text-slate-500">{label}</p>
    <p className={`mt-2 font-code text-2xl font-semibold ${accent}`}>{value ?? 0}</p>
  </div>
);
...
function SandboxPage() {
  ...
  const selectedSubmission = useMemo(
    () => submissions.find((item) => String(item.submission_id) === String(selectedSubmissionId)),
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const submissionRows = Array.isArray(submissionsRes.data) ? submissionsRes.data : [];
  ...
  return (
    <div className="min-h-screen bg-abyss px-6 py-10 text-slate-100 md:px-10 lg:px-16">
      <div className="mb-8 flex flex-col gap-4 border-b border-phantom pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="mb-3 flex items-center gap-2">
            ...
            <h1 className="text-3xl font-black uppercase tracking-tight text-slate-100">Sandbox Execution</h1>
          </div>
          <p className="max-w-3xl text-sm text-slate-400">
            Submit a known file hash for sandbox intelligence and persist the result against a Contagion submission.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 rounded border border-phantom bg-obsidian px-4 py-3">
          ...
          <span className="font-code text-[10px] uppercase tracking-[0.24em] text-toxic">Sandbox Ready</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(360px,440px)_1fr]">
        <form onSubmit={handleRun} className="glass-panel rounded-lg p-5 shadow-card">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="font-display text-sm font-bold uppercase tracking-[0.22em] text-slate-200">Execution Parameters</h2>
          ...
          <label className="mb-4 block">
            <span className="mb-2 block font-code text-[10px] uppercase tracking-[0.2em] text-slate-500">Submission</span>
            <select
          ...
          <label className="mb-4 block">
            ...
            <input
              ...
              placeholder={selectedHasHash ? selectedSubmission.sha256_hash : 'MD5, SHA-1, or SHA-256'}
          ...
          <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label>
              <span className="mb-2 block font-code text-[10px] uppercase tracking-[0.2em] text-slate-500">Environment</span>
              <select
                value={environment}
            ...
            <label>
              <span className="mb-2 block font-code text-[10px] uppercase tracking-[0.2em] text-slate-500">OS Profile</span>
              <select
                value={osProfile}
          ...
          <label className="mb-5 block">
            <span className="mb-2 block font-code text-[10px] uppercase tracking-[0.2em] text-slate-500">Timeout Seconds</span>
            <input
              type="number"
              min="30"
              max="600"
          ...
          <label className="mb-6 flex cursor-pointer items-center justify-between rounded border border-phantom bg-void px-4 py-3">
            <span>
              <span className="block font-code text-[10px] uppercase tracking-[0.2em] text-slate-400">Network Enabled</span>
              <span className="text-xs text-slate-600">Stored with the execution metadata</span>
            </span>
            <input
              type="checkbox"
        ...
        <section className="space-y-6">
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <CountTile label="Malicious" value={latestStats?.malicious} accent="text-red-400" />
            <CountTile label="Suspicious" value={latestStats?.suspicious} accent="text-amber-400" />
            <CountTile label="Harmless" value={latestStats?.harmless} accent="text-green-400" />
            <CountTile label="Undetected" value={latestStats?.undetected} accent="text-slate-300" />
          </div>

          <div className="glass-panel rounded-lg p-5 shadow-card">
            <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <h2 className="font-display text-sm font-bold uppercase tracking-[0.22em] text-slate-200">Latest Evaluation</h2>
              {latestSeverity && (
                <span className={`w-fit rounded border px-3 py-1 font-code text-[10px] uppercase tracking-widest ${severityStyles[latestSeverity]}`}>
                  {latestSeverity}
                </span>
              )}
            </div>
            ...
            {latestResult ? (
              <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
                <div className="rounded border border-phantom bg-void p-4">
                  <p className="font-code text-[10px] uppercase tracking-[0.2em] text-slate-500">Artifact</p>
                  <p className="mt-2 text-sm font-semibold text-slate-100">{latestResult.file?.name || 'Unnamed sample'}</p>
                  <p className="mt-3 break-all font-code text-[11px] text-slate-500">{latestResult.file?.sha256}</p>
                  <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <p className="font-code uppercase tracking-widest text-slate-600">Type</p>
                      <p className="mt-1 text-slate-300">{latestResult.file?.type || 'Unknown'}</p>
                    </div>
                    <div>
                      <p className="font-code uppercase tracking-widest text-slate-600">Size</p>
                      <p className="mt-1 text-slate-300">{latestResult.file?.size || 0} bytes</p>
                    </div>
                  </div>
                </div>

                <div className="rounded border border-phantom bg-void p-4">
                  <p className="font-code text-[10px] uppercase tracking-[0.2em] text-slate-500">Behavior Summary</p>
                  <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                    <CountTile label="Domains" value={latestResult.behavior?.contacted_domains?.length} accent="text-cyan-300" />
                    <CountTile label="Processes" value={latestResult.behavior?.processes_created?.length} accent="text-slate-200" />
                    <CountTile label="Files Written" value={latestResult.behavior?.files_written?.length} accent="text-slate-200" />
                  </div>
          ...
          <div className="glass-panel rounded-lg p-5 shadow-card">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-sm font-bold uppercase tracking-[0.22em] text-slate-200">Execution History</h2>
              <span className="font-code text-[10px] uppercase tracking-[0.2em] text-slate-600">{executions.length} records</span>
            </div>
              ...
              <table className="w-full min-w-[760px] text-left">
                ...
                <tbody>
                  {executions.map((execution) => (
                    <tr key={execution.execution_id} className="border-b border-phantom/70 text-sm text-slate-300">
                      <td className="py-4 font-code text-slate-500">#{execution.execution_id}</td>
                      ...
                      <td className="py-4">
                        ...
                        <p className="mt-1 max-w-[220px] truncate font-code text-[10px] text-slate-600">{execution.sha256_hash}</p>
                      </td>
                      <td className="py-4 font-code text-xs text-slate-500">{toDateTime(execution.finished_at)}</td>
