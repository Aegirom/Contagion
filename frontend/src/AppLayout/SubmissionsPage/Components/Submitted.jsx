...
function Submitted({
  ...
  return (
    <div className="bg-obsidian border border-phantom rounded-lg p-6 shadow-xl transition-all hover:border-toxic/30 group animate-fade-up">
      {/* Upper Part: Title and Status */}
      <div id="submittedUpperPart" className="mb-4">
        <div
          id="submittedTitleLine"
          className="flex justify-between items-start mb-2"
        >
          <h2
            id="submittedName"
            className="text-gray-900 text-2xl font-bold tracking-tight truncate mr-3"
          >
            {name}
          </h2>
          <p
            id="submittedStatus"
            className="text-gray-500 text-[10px] uppercase tracking-[0.2em] font-black pt-1 whitespace-nowrap"
          >
            {status}
          </p>
        </div>
      ...
      <div id="submittedTags" className="flex gap-2 mb-5 flex-wrap">
        <p
          id="submittedFamily"
          className="text-toxic bg-green-50 border border-green-200 px-3 py-1 rounded text-xs font-mono uppercase"
        >
          {family}
        </p>
        <p
          id="submittedThreatLevel"
          className={`px-3 py-1 rounded text-xs font-mono uppercase border whitespace-nowrap ${getThreatColor(threatLevel)}`}
        >
          THREAT: {threatLevel}
        </p>
        {sandboxStatus && (
      ...
      <div className="mb-5 rounded-md border border-phantom/50 bg-gray-100/40 px-4 py-3">
        <p className="font-code text-[10px] uppercase tracking-widest text-gray-500 truncate">
          {fileName || "No artifact linked"}
        </p>
        <p className="mt-1 truncate font-code text-[11px] text-toxic">
          {hash || "Upload an artifact to enable sandbox execution"}
        </p>
      </div>
      ...
      <div className="mb-5 bg-gray-100/50 px-4 py-3 rounded-md border border-phantom/50">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-viral font-bold uppercase tracking-widest">
              Neural Analysis
            </span>
          </div>
          <button
