  ...
  if (error) {
    return (
      <div className="min-h-screen bg-abyss text-slate-100 px-6 py-12 md:px-12 lg:px-20 text-center">
        ...
        <p className="text-slate-400 text-sm font-mono mb-8">{error}</p>
        <div className="flex justify-center gap-4">
          <button 
            onClick={() => loadEvaluation()}
            className="px-6 py-2 bg-phantom hover:bg-phantom/80 text-slate-100 font-mono text-xs uppercase tracking-widest"
          >
            Retry Analysis
          </button>
          <button 
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-row justify-between items-end pb-6 border-b border-phantom">
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-sm font-mono">{evaluation.submissionName || `Submission #${submissionId}`}</p>
          </div>
